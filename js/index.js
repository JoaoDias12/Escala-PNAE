...
function distribuirPessoas() {
  ...
  function addGateMakerToCheckin(day, dayDiv) {
    ...
    if (gateMakerForDay) {
      const flight0950People = assignments[dateString]['Nova York - 0950']
      ...
      gateMakerDiv.innerHTML = `<h2>Fechamento De Check-In</h2><h3>${
  ...
  for (let day = diaAtual.getDate(); day <= daysInMonth; day++) {
    ...
    // List free people
    const freePeopleDiv = document.createElement('div')
    freePeopleDiv.className = 'freePeople'
    freePeopleDiv.innerHTML = '<h3>Pessoas Livres:</h3>'
    const freePeople = Object.keys(peoples).filter(personKey => {
      const person = peoples[personKey]
      const isIn0950 =
        assignments[dateString]['Nova York - 0950'].includes(personKey)
      const isGateMaker =
        assignments[dateString]['GateMaker']?.includes(personKey)
      return (
    ...
    dayDiv.appendChild(freePeopleDiv)
...
function salvarAssignments(assignments, diaAtual) {
  ...
  if (totalNomes >= 3 && diaAtual) {
    let dateKey = diaAtual.toISOString().split('T')[0] // Formatar a data como YYYY-MM-DD
    firebase.database().ref(`app/assignments/${dateKey}`).set(assignments)
